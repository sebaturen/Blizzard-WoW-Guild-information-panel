...
<%if(guildMember) {%>
    ...
    <%  List<KeystoneAffix> kAfixIndex = new ArrayList<>();
        int i = 0;
        for(KeystoneDungeonRun keyRun : mPlus.getWeekFailRun()) { int[] durTime = keyRun.getTimeDuration(); %>
            keystone_run [<%= i++ %>] = {
                'run_id': '<%= keyRun.getId() %>',
    ...
    <%  } //foreach keyRun
        ...
        i = 0;
        for(KeystoneDungeonRun keyRun : mPlus.getLastBestFailKeyRun()) { int[] durTime = keyRun.getTimeDuration(); %>
            keystone_best_run [<%= i++ %>] = {
                'run_id': '<%= keyRun.getId() %>',
                'key_lvl': '<%= keyRun.getKeystoneLevel() %>',
                'complete_date': '<%= keyRun.getCompleteDate() %>',
